...
try:
    while True:
        ...
        color_image=cv2.resize(color_image, (newsize,newsize))[half_newsize-240:half_newsize+240,half_newsize-240:half_newsize+240]
        if(center_mean>270):
        ...
        #imgforCNN=screen
        #print(screen.shape)
        color_image=scipy.ndimage.rotate(color_image,angle,reshape=False)
        if timer%5==0:
            result= process_img(color_image)
            resultp=process_img(scipy.ndimage.rotate(color_image,10,reshape=False))
            resultn=process_img(scipy.ndimage.rotate(color_image,-10,reshape=False))
        ...
        text="GUESSVALUE:%f - GRASPABLE:%d"%(prob,label)
